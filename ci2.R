  ...
  for (j in 1:3) {
    for (theta in c(0, 1)) {
      for (k in c(2,3,4,5,7)) {
        ...
        mod.p.df <- data.frame(matrix(NA, nrow=n.rep, ncol=4))
        ...
        colnames(mod.p.df) <- c("est", "se", "low", "hi")
        for (b in 1:n.rep) {
          ## model fitting
          ...
          if(!(inherits(mod, "try-error"))){
            ...
            if(!(inherits(mod0, "try-error"))){
              suppressWarnings({
                mod.p.pv <- try(permlmer.ci(mod0, mod, nperm = n.perm, ncore=16,data=df.ma.lst[[b]]), silent=TRUE) ## thien phuc long hao
              })
          ...
          if(b%%10==0){
            #save.image(paste("pvalue_epsilonnorm_theta", theta, "_n", n, "_tau", tau[k], "_n.subject", n.subject[j], "_nperm2000_lmer_ci_TPLH2.RData", sep=""))
            #save.image(paste("pvalue_epsilonlognorm_theta", theta, "_n", n, "_tau", tau[k], "_n.subject", n.subject[j], "_nperm2000_lmer_ci_TPLH2.RData", sep=""))
            #save.image(paste("pvalue_diffuisd_theta", theta, "_n", n, "_tau", tau[k], "_n.subject", n.subject[j], "_nperm2000_lmer_ci_TPLH2.RData", sep=""))
            #save.image(paste("pvalue_epsilont_theta", theta, "_n", n, "_tau", tau[k], "_n.subject", n.subject[j], "_nperm2000_lmer_ci_TPLH2.RData", sep=""))
